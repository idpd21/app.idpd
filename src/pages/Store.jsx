...
function Store() {
  ...
  const handleUpload = async () => {
    ...
    formData.append("upload_preset", "image upload"); // ใส่ชื่อ Upload Preset

    try {
      const res = await axios.post(
        "https://api.cloudinary.com/v1_1/dgk7z5wwz/image/upload",
        formData
      );
                    ...
                    <tr key={item.id} className="border border-green-700">
                      <td className="py-2 px-4 border border-green-700 text-center">
                        {item.image ? (
                          ...
                          <img
                            src="https://cdn-icons-png.flaticon.com/512/259/259987.png"
                            className="h-20 w-20"
                          />
                        )}
                      </td>
